...
- `release.yml` is generated by `cargo dist generate` and owned
  upstream: never hand-edit it. After regenerating, re-pin every
  `uses:` line to SHAs (same format) before merging — the SHA check
  scans generated code too, and will fail the PR if you forget.
