...
| `x` | discard the hunk from the working tree (ADR-0014) — Margin's only destructive action. Opens a prompt; only typing `yes` and Enter applies it, Esc cancels. A backup patch lands in `.git/margin/trash/` first (`margin undo` restores). Worktree reviews only. Pass `--discard-without-backup` to skip the recovery copy for one invocation (ADR-0017); the prompt says so. |
